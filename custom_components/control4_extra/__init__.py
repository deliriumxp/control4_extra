...
#
# Difference from upstream: PLATFORMS is computed per config entry from
# `CONF_ENABLED_PLATFORMS` (set via the options flow) instead of being a fixed
# module-level list of all four platforms - so a household only gets the
# Control4 domains it actually wants surfaced in HA.
...
from .vendor.pycontrol4.error_handling import BadCredentials
...
def _enabled_platforms(entry: ConfigEntry) -> list[Platform]:
    values = entry.options.get(CONF_ENABLED_PLATFORMS, DEFAULT_ENABLED_PLATFORMS)
...
async def async_setup_entry(hass: HomeAssistant, entry: Control4ConfigEntry) -> bool:
    """Set up Control4 Extra from a config entry."""
    ...
    try:
        # pyControl4 2.x returns already-parsed JSON (unlike 1.x, which
        # returned the raw response text).
